/**
 * Google Drive Cloud Sync for olaylar session snapshots (appDataFolder).
 *
 * Architecture (adapted from notepad's `js/cloud-sync.js` to olaylar's
 ...
 *   - Layout:
 *       olaylar-index.json        → { version, lastSync, sessions: [{id,name,updated,bytes,rev}] }
 *       olaylar-session-<id>.json → { meta: RecentSessionMeta, payload: save_session JSON string }
...
const LS_TOKEN = "olaylar_cloud_token";
const LS_USER = "olaylar_cloud_user";
const LS_LAST_SYNC = "olaylar_cloud_last_sync";
const LS_MODE = "olaylar_cloud_mode"; // 'popup' | 'redirect'
const SS_STATE = "olaylar_oauth_state"; // CSRF state (sessionStorage)
...
async function uploadJson<T>(
  ...
  const boundary = "-------OLAYLARCloud" + Math.random().toString(36).slice(2);
  const body =
    `--${boundary}\r\n` +
...
interface RemoteIndexEntry {
  id: string; // local olaylar id (matches sessionDb RecentSessionRecord.id)
  name: string;
  updated: number; // epoch ms (LWW clock — matches sessionDb savedAt)
  bytes: number;
  rev: string | null; // Drive file id of olaylar-session-<id>.json
...
/** Save a user-provided Client ID and re-initialize cloud sync so the
 *  sign-in button becomes active immediately. */
export async function setClientId(id: string | null): Promise<void> {
  const hadClientId = !!getClientId();
  saveClientId(id);
  const hasClientId = !!getClientId();

  if (hadClientId && !hasClientId) {
    // User cleared the Client ID: sign out if currently connected.
  ...
  if (hasClientId) {
    // Reset init flag so init() can run again with the new Client ID.
    initialized = false;
    await init();
  }
}
