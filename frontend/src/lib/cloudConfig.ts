/**
 * Google OAuth client configuration for olaylar cloud sync.
 *
 * Mirrors notepad's `js/cloud-config.js`, adapted for olaylar's session model.
 *
 ...
 *            https://olaylar.drtr.uk          (web deployment)
 ...
 *   - olaylar's Tauri desktop webview runs at http://127.0.0.1:<port> (NOT a
 ...
 *     ⇒ same olaylar sessions.
 */

const LS_CLIENT_ID = "olaylar_cloud_client_id";

/** Return the active Google OAuth Client ID.
 *
 * Priority:
 *   1. User-provided Client ID saved in localStorage (per-device, editable in UI).
 *   2. Hard-coded Client ID in CLOUD_CONFIG.GOOGLE_CLIENT_ID (app owner / build-time).
 */
export function getClientId(): string {
  try {
    const userId = localStorage.getItem(LS_CLIENT_ID);
    if (userId?.trim()) return userId.trim();
  } catch {
    /* ignore */
  }
  return CLOUD_CONFIG.GOOGLE_CLIENT_ID;
}

/** Save (or clear) a user-provided Google OAuth Client ID in localStorage. */
...
export const CLOUD_CONFIG = {
  ...
  // This is the fallback; users can also enter their own Client ID in the app UI.
  GOOGLE_CLIENT_ID: "",
...
//   olaylar-session-<id>.json → { meta: {...}, payload: "<save_session JSON string>" }
export const INDEX_FILE_NAME = "olaylar-index.json";
export const sessionFileName = (id: string): string => `olaylar-session-${id}.json`;
