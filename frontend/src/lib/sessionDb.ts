/**
 * Local-only autosave for olaylar sessions.
 *
 * Mirrors the endnotere editor pattern (Dexie / IndexedDB store + 600 ms
 * debounce + cross-tab BroadcastChannel) but adapted for olaylar's
...
import type { RecentSessionMeta } from "../types";

// Re-export so existing callers can still import from sessionDb.ts.
...
function getDb(): SessionDB {
  ...
  if (_db) return _db;
  const db = new Dexie("olaylar-sessions-v1") as SessionDB;
  db.version(1).stores({
...
const CHANNEL = "olaylar-sessions";
