/**
 * CloudSyncBar — Google Drive sync status indicator for the olaylar header.
 *
...
export default function CloudSyncBar() {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (!target.closest("[data-cloud-menu]")) {
        setMenuOpen(false);
        setSetupOpen(false);
      }
    };
      ...
      <>
        ...
        >
          ...
          <span className="hidden sm:inline">
            {info.status === "setupNeeded" ? "Drive Ayarla" : "Drive Bağla"}
          </span>
        </button>
        {setupOpen && (
          <CloudClientIdSetup variant="bar" onDone={() => setSetupOpen(false)} />
        )}
      </>
    );
  }
